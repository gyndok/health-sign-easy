-- ============================================================
-- Fix: handle_new_user trigger now also creates user_profiles rows
-- Fix: onboarding RPCs use INSERT...ON CONFLICT for user_profiles
-- ============================================================

-- Update handle_new_user to also create user_profiles
...
-- ============================================================
-- Fix save_provider_onboarding: INSERT...ON CONFLICT for user_profiles
-- ============================================================
...
BEGIN
  ...
  -- Ensure user_profiles exists, then update it
  -- First try to get the org_id from existing row
  SELECT org_id INTO v_org_id FROM user_profiles WHERE id = v_user_id::text;
  ...
  IF v_org_id IS NULL THEN
    ...
    BEGIN
      ...
      -- If org already existed from a previous attempt, find it
      ...
      VALUES (
        ...
        'provider',
        COALESCE(v_new_org_id::text, v_user_id::text),
        v_email,
        p_phone,
        p_practice_name,
        p_primary_specialty,
...
-- ============================================================
-- Fix save_patient_onboarding: INSERT...ON CONFLICT for user_profiles
-- ============================================================
...
-- ============================================================
-- Also fix update_provider_profile to handle missing user_profiles
-- ============================================================
